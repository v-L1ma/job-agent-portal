...
export function PreferenciasForm({ mode = "page", onSaved }: PreferenciasFormProps) {
  ...
  return (
    <div className={isModal ? "space-y-6" : "max-w-3xl mx-auto pb-12 space-y-8"}>
      {!isModal && (
        <header>
      ...
      <section className="space-y-3">
        ...
        <div className="p-5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50">
          <div className="flex justify-between items-start mb-4 flex-wrap">
            ...
            <div className="text-xs text-slate-500 font-medium">{keywords.length} / 5</div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="flex-1 space-y-1">
              <Input
                ...
                className={newKeyword.length > 30 || errors.skills ? "border-red-500 focus-visible:ring-red-500" : ""}
                disabled={keywords.length >= 5}
              />
              <div className="flex justify-between px-1">
                <p
                  className={`text-[10px] uppercase font-bold tracking-wider ${
                    newKeyword.length > 30 ? "text-red-500" : "text-slate-400"
                  }`}
                >
      ...
      <section className="space-y-4 p-5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50">
        <div className="flex flex-col gap-1">
          <Label className="text-base font-bold">
            Senioridades <span className="text-red-500">*</span>
          </Label>
          <p className="text-sm text-slate-500">Selecione os níveis de experiência para as vagas.</p>
        </div>

        <div className="grid gap-3">
          {SENIORITY_OPTIONS.map((option) => (
            <div key={option} className="flex items-center space-x-3 cursor-pointer group">
              <Checkbox
                id={`level-${option}`}
      ...
      <div className="pt-4 border-t border-slate-200 dark:border-slate-800 flex justify-end gap-3">
        ...
        >
          {saving ? (
            ...
            isModal ? "Salvar e continuar" : "Salvar Preferências"
          )}
        </Button>
      </div>
    </div>
